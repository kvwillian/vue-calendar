<template>
  <div class="relative">
    <label class="block text-sm mb-1 text-gray-600 dark:text-gray-300">City</label>
    <input
      ref="inputEl"
      v-model="query"
      :placeholder="modelValue ? label(modelValue) : 'Type a city name...'"
      class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100"
      @focus="open = true"
    ...
    <ul
      ...
      class="absolute z-50 mt-1 w-full bg-gray-200 dark:bg-gray-700 border dark:border-gray-600 border-gray-300 rounded shadow-lg max-h-60 overflow-auto"
    > 
      <li
        v-for="(loc, i) in results"
        :key="loc.lat + ',' + loc.lon"
        :class="['px-3 py-2 text-sm cursor-pointer', i === activeIndex ? 'bg-gray-100 dark:bg-gray-600' : 'hover:bg-gray-200 dark:hover:bg-gray-600']"
