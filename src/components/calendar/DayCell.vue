<template>
  ...
  <div :class="['bg-white dark:bg-gray-800 p-1 min-h-[110px] relative cursor-pointer select-none',
    { 'opacity-60': !inMonth }, hoverable && 'hover:bg-gray-50 dark:hover:bg-gray-700']" @click="$emit('new', iso)" role="button"
    ...
    <div class="text-xs font-medium mb-1 text-gray-900 dark:text-gray-100">{{ dayNumber }}</div>
    ...
    <ul v-if="reminders.length">
      <li v-for="r in visible" :key="r.id" class="mb-1">
        <button class="w-full text-left truncate px-1 py-0.5 rounded flex items-center gap-1 hover:bg-gray-100 dark:hover:bg-gray-600"
          ...
          <span class="text-[11px] tabular-nums text-gray-700 dark:text-gray-300">{{ r.time }}</span>
          <span class="truncate text-[11px] text-gray-700 dark:text-gray-300">{{ r.text }}</span>
