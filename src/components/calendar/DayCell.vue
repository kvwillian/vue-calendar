<template>
  <!-- toda a célula vira um botão -->
  <div :class="['bg-white p-1 min-h-[110px] relative cursor-pointer select-none',
    { 'opacity-60': !inMonth }, hoverable && 'hover:bg-gray-50']" @click="$emit('new', iso)" role="button"
    tabindex="0" @keydown.enter="$emit('new', iso)">
    <button class="absolute top-1 right-1 text-xs px-1 py-0.5 rounded hover:bg-gray-100" aria-label="Add reminder"
      @click.stop="$emit('new', iso)">＋</button>

    <div class="text-xs font-medium mb-1">{{ dayNumber }}</div>

    <ul v-if="reminders.length">
      <li v-for="r in visible" :key="r.id" class="mb-1">
        <button class="w-full text-left truncate px-1 py-0.5 rounded flex items-center gap-1 hover:bg-gray-100"
          :style="{ borderLeft: `4px solid ${r.color}` }" @click.stop="$emit('open-reminder', r)">
          <span class="text-[11px] tabular-nums">{{ r.time }}</span>
          <span class="truncate text-[11px]">{{ r.text }}</span>
          <span v-if="r.weather" class="text-[10px] italic ml-auto flex items-center gap-1">
    ...
    <button v-if="overflow" class="text-[11px] underline" @click.stop="$emit('open-day', iso)">
      +{{ overflow }} more
    </button>
  </div>
</template>
