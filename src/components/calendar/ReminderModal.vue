<template>
  ...
  >
    ...
    >
      ...
      <h3 id="reminder-title" class="text-lg font-semibold text-gray-800 dark:text-gray-100">
        {{ isEdit ? 'Edit reminder' : 'New reminder' }}
      </h3>

      <label class="block text-sm text-gray-600 dark:text-gray-300">
        <span class="block mb-1">Date</span>
        <input type="date" v-model="form.dateISO" class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100" required>
      </label>

      <label class="block text-sm text-gray-600 dark:text-gray-300">
        <span class="block mb-1">Time</span>
        <input type="time" v-model="form.time" class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-100" required>
      </label>

      <label class="block text-sm text-gray-600 dark:text-gray-300">
        <span class="block mb-1">Text (max 30)</span>
        <input v-model.trim="form.text" maxlength="30" class="w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100" required>
      </label>

      <CitySelect v-model="form.loc" />

      <div class="block text-sm text-gray-600 dark:text-gray-300">
      ...
      <div class="pt-2 flex gap-2">
        ...
        <button type="button" class="px-3 py-1.5 rounded border border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300" @click="$emit('close')">Cancel</button>
        <button
          v-if="isEdit"
          type="button"
          class="ml-auto px-3 py-1.5 rounded border border-gray-300 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-700 text-red-600 dark:text-red-400"
          @click="del"
        >Delete</button>
      </div>
    </form>
